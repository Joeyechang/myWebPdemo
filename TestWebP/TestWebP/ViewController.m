...
- (void)viewDidLoad
{
    ...
    jpgImages  = [[NSBundle mainBundle] pathsForResourcesOfType:@"jpg" inDirectory:nil];
    
    _bypassFilteringSwitch.on = NO;
    _noFancyUpsamplingSwitch.on = NO;
    _useThreadsSwitch.on = NO;
    
    _bypassFilteringSwitch.hidden = YES;
    _noFancyUpsamplingSwitch.hidden = YES;
    _useThreadsSwitch.hidden = YES;
...
-(void)displayJpgImage:(NSInteger)index{
    
    CGRect webpFrame = self.imageScrollView.frame;
    webpFrame.origin.y = 268;//+= webpFrame.size.height + 40;
    self.imageScrollView2.frame = webpFrame;
    NSMutableString * jpgName = [jpgImages objectAtIndex:index];
    NSArray * arr = [jpgName componentsSeparatedByString:@"/"];
    jpgName = [arr lastObject];
    self.testImageView2.image = [UIImage imageNamed:jpgName];
   
    self.testImageView2.backgroundColor = [UIColor orangeColor];
    
    self.imageScrollView2.contentSize = webpFrame.size;
...
- (void)displayImage:(NSString *)filePath
{
    ...
    self.testImageView.frame = CGRectMake(0, 0, width/2, height/2);
    self.testImageView.image = newImage;
    
    self.imageScrollView.contentSize = CGSizeMake(width/2, height/2);
